...
//                '</tpl>'
	...
	var index = this.calendarStore.findExact(M.CalendarId.name, evt[M.CalendarId.name]);
	
        data.Title = (evt[M.IsAllDay.name] ? '' : 
		      Ext.Date.format(evt[M.StartDate.name], fmt) + '-'
		      + Ext.Date.format(evt[M.EndDate.name], fmt)
		      + this.calendarStore.getAt(index).data.Title
		      // + evt[M.CalendarId.name]
		     ) ;
