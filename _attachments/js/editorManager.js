...
({factory: function() {
    ...
    var editorWindow = isc.Window.create({
        ...
        removeCanvas: function() {
            ...
            this.removeItem(this.getCanvas());
        }
    });
    ...
    var fill = function(container, record, settings) {
        ...
        if (!record.group) {
            console.error("This record doesn't have group/type", record);
            return false;
        }
        var editor = editors[record.group];
        if (!editor) {
        ...
        if (editor.canvas !== container.getCanvas()) {
            if (containers[record.group])
                containers[record.group].removeCanvas();
            container.removeCanvas();
            container.setCanvas(editor.canvas); 
            containers[record.group] = container;
        }
