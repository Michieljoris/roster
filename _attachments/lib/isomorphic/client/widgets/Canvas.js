...
getTagStart : function (dontConcat) {
    ...
    // adjacentHTML we write out for the accessKeyProxy. *Note*: as at now (March 2012), certain 
    // key-combinations that are used internally by Chrome seem not to be useable by SC.
...
_getHoverProperties : function () {
    var target = isc.EH.getTarget(),
        parent = isc.isA.CanvasItem(target) ? target : target.canvasItem,
        result
    ;
...
imgHTML : function (src, width, height, name, extraStuff, imgDir, activeAreaHTML, 
    ...
    // We single-quote the URL - escape any ' characters!
    } else {
