...
_handleNativeKeyDown : function (DOMevent, fromOnHelp) {
    ...
    // For IE we *always* fire a synthetic keyPress in response to a keyDown event for certain keys
    
    var mustFireKeyPress = isc.Browser.isIE &&
    ...
    if (!mustFireKeyPress && EH._keyDownKeyNames.indexOf(lastEvent.keyName) != -1) {
...
handleKeyDown : function (nativeEvent, scEventProperties) {
    ...
    var useSyntheticTabIndex = false;
    if (EH._focusCanvas && EH._focusCanvas.useSyntheticTabIndex)
    {
        useSyntheticTabIndex = true;
    }
    if (returnVal != false && 
        (useSyntheticTabIndex || this.clickMaskUp()) && lastEvent.keyName == this._$Tab) 
    {
        var topHardMask,
        ...
        if (useSyntheticTabIndex || topHardMask) returnVal = false;
        if (useSyntheticTabIndex) {
            this.logInfo("Widget:" + EH._focusCanvas + " explicitly marked as requesting synthetic " +
                "tab index mgmt. Intercepting keyDown to achieve this.", "syntheticTabIndex");
            // Remember the "source" of the event. Then when keyPress fires even if focus has
            // been taken from it, we can still force focus into the next widget in the 
            // page's tab order.
            EH.syntheticTabSource = EH._focusCanvas;
        }
    }
...
handleKeyPress : function (nativeEvent, scEventProperties) {
    ...
    if ((EH.syntheticTabSource || this.clickMaskUp()) && lastEvent.keyName == this._$Tab) {
        var topHardMask,
            registry = this.clickMaskRegistry;
        for (var i = registry.length-1; i >=0; i--) {
            if (this.isHardMask(registry[i])) {
        ...
        if (EH.syntheticTabSource) {
            this.logInfo("Telling canvas:" + EH.syntheticTabSource + 
                    " [marked as 'useSyntheticTabIndex:true'] to shift focus", 
                            "syntheticTabIndex")

            EH.syntheticTabSource.delayCall("_focusInNextTabElement", [!this.shiftKeyDown(), topHardMask]);
            delete EH.syntheticTabSource;
            return false;

        } else if (topHardMask != null) {
    ...
    function (e) {
        ...
        if (isc.Browser.isMobileWebkit) { 
		    ...
		    if (isc.startsWith(scEvent.eventType, "touch")) {
                ...
                scEvent.x = parseInt(e.clientX) + isc.Page.getScrollLeft();
...
//
// @param target (Canvas) target of the event.
