...
getEmptyMessageHTML : function (startCol,endCol,offline) {
    ...
    sb.append(
          "<TABLE role='presentation' BORDER=0 MARGIN=0 CELLSPACING=0",
                (this.emptyMessageTableStyle?(" CLASS='" + this.emptyMessageTableStyle + "'"):""),
...
getTableHTML : function (colNum, startRow, endRow, discreteCols, asyncCallback, isAsync) {
    ...
    if (this.isEmpty()) {
        ...
        if (this.isPrinting) {
            ...
            if (this._printingChunk && this.printChunkOnly) return this._showEmptyMessage();
            
            return this.grid.getPrintHeaders(startCol, endCol) + this._showEmptyMessage() 
                    + this.grid.getPrintFooters(startCol, endCol);
        }
...
//                        component + " went asynchronous", "printing");
	...
	if (!this._printingChunk || (endRow == this.getTotalRows() && !this.printChunkOnly)) {
	    if (this.grid && this.isPrinting) {
	        output.append(this.grid.getPrintFooters(startCol, endCol));
        }
        output.append("</TABLE>");
    }
                ...
                value = [value, 
                        ...
                        // cell included a block level element
                            ...
                            isc.Canvas.spacerHTML(1, details.extraHeight-
                                Math.max(isc.ListGrid.getInstanceProperty("cellHeight"),this.cellHeight)
                            ),
...
selectOnMouseUp : function (record, rowNum, colNum) {
    ...
    if (rowNum >= 0 && colNum >= 0) {
        ...
        this.grid._dontRefreshSelection = true;
        var selectionChanged = this.selection.selectOnMouseUp(this, rowNum, colNum);
        this.grid._dontRefreshSelection = null;
...
//    this.logWarn("Logic to get col autoSize running:" + [columnNum, startRow, endRow]);
    ...
    // No need to clear the columnSizer: It's drawn offscreen already and if we clear we'll have
    // to re-draw next time we size a column which can end up with a huge number of expensive
    // clear-draw's for a grid with lots of columns
//    columnSizer.clear();
