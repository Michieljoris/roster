...
cacheSelection : function () {
    ...
    if (this._cachingSelection || this._settingSelected || this._supressCaching) return;
...
selectList : function (list, newState) {
    ...
    for (var i = 0; i < length; i++) {
        ...
        if (!newState) {
            currentIndex[selectionChanged.length-1] = cache.indexOf(item);
        }
    }
...
// Deselect all records in the list
//
// @return (boolean) true == selection actually changed, false == no change
// @group selection
// @visibility external
