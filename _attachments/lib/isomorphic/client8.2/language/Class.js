...
isc.Class.addClassMethods({
    ...
    evaluate : function (expression, evalArgs, globalScope, isJSON) {
        //!OBFUSCATEOK
        ...
        if (isJSON && isc.Browser.isIE && !globalScope && isc.Page.isLoaded()) {
	...
	evalInIFrame : function (expression, evalArgs) {
        ...
        return this.evalFrame.doEval(expression, evalArgs);
	},
	
	makeEvalFrame : function () {
	    this.evalFrame = isc.HiddenFrame.create({
	    ...
	    this.evalFrame.draw();
	    // resetEvalFrame sets up the doEval method.
	    this.resetEvalFrame();	    
	},
	
    evalFrameHTML:"<html><body><script>" +
	...
	resetEvalFrame : function () {
	    ...
	    var frame = this.evalFrame.getFrameDocument();
	    frame.open();
	    frame.write(this.evalFrameHTML);
	    frame.close();
    },	
...
// @visibility external
//<
// Additional 'isJSON' param indicating that we're evaluating a JSON block
...
isc.eval = function (expression, isJSON) {
    return isc.Class.evaluate(expression, null, false, isJSON);
}
