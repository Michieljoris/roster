 ...
 { "use strict";
   ...
   function log() {
       ...
       if (level <= verbosity) {
           ...
           if (consoleObj) consoleObj.apply(console, args);
   ...
   function timedOut() {
	   ...
	   noresponse.forEach(function(r) {
               var arr = [];
               r.requirers.forEach(function(req) {
                   arr.push(req.namespace);
               });
	       log(W,r.id + ' is required by ' + arr); }); } 
       noresponse = [];
   ...
   function getObject(ns, path, value) {
       ...
       // log(E, 'making namespace for ', path, ns, value);
       if (path) {
   ...
   function finalize() {
	   ...
	   function (dep) {
	       ...
	       // log(D,'def=' , def);
	       log(I,dep.namespace + " is needed in " +  
   ...
   function execJasmine() {
       console.log('Testing with jasmine');
